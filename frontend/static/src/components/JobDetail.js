...
class Comment extends React.Component {
  ...
  render() {
    ...
    return (<form onSubmit={(e) => this.props.addComment(e, this.state.text)}>
...
class JobDetail extends React.Component {
  ...
  deleteComment = (comment) => {
    ...
    axios.delete(`/api/v1/comments/${comment.id}`).then(response => {
      // handle success
  ...
  getJob = () => {
    axios.get(`/api/v1/jobs/${this.props.match.params.id}`).then(response => {
      // handle success
  ...
  render() {
    ...
    return (<div>
      <button className="btn-light" value='edit'>
        <NavLink className="edit" to={`/jobs/edit/${this.props.match.params.id}`}>Edit</NavLink>
      </button>
